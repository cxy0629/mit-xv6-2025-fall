...
int e1000_transmit(char *buf, int len)
{
  ...
  uint32 tail = regs[E1000_TDT]; // 发送描述符指针尾指针

  if ((tx_ring[tail].status & E1000_TXD_STAT_DD) == 0) // 发送描述符环满
  {
    release(&e1000_lock);
    return -1;
  }

  if(tx_ring[tail].addr != 0) // 地址不为0且描述符可用，释放之前的缓冲区
  {
...
static void e1000_recv(void)
{
  while(1){
    ...
    uint32 tail = (regs[E1000_RDT] + 1) % RX_RING_SIZE; // 接收描述符尾指针 
    ...
    char *buf = (char *)rx_ring[tail].addr;
    int len = rx_ring[tail].length;

    // 描述符就绪，处理内核缓冲区中的数据包
    rx_ring[tail].status = 0;
    char *newbuf = kalloc(); // 分配新缓冲区
    if(newbuf == 0)
      panic("e1000_recv: kalloc");
    rx_ring[tail].addr = (uint64)newbuf;

    regs[E1000_RDT] = tail; // 更新尾指针
    release(&e1000_lock);

    net_rx(buf, len); // 交给上层网络协议栈处理数据包
  }
}
